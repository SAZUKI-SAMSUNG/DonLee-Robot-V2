...
async def start(bot: DonLee_Robot_V2, msg: Import.Msg):
    START_BUTTON = [[  
          Import.Button("𝙰𝙳𝙳 𝙼𝙴 𝚃𝙾 𝙰 𝙲𝙷𝙰𝚃 𝙶𝚁𝙾𝚄𝙿", url=f"http://t.me/{Config.BOT_USERNAME}?startgroup=true")
        ...
        try:
            ...
            await msg.reply_text(
                ...
                reply_markup=Import.Markup([
                    [ Import.Button(text="𝙹𝙾𝙸𝙽", url=f"https://t.me/{Config.FORCE_CHANNEL}"),
                      Import.Button(text="𝚃𝚁𝚈 𝙰𝙶𝙰𝙸𝙽", url=f"https://t.me/{Config.BOT_USERNAME}?start={file_uid}")]       
              ])
            )
            return
    ...
    button = [[
     Import.Button('𝙰𝙳𝙳 𝙼𝙴 𝚃𝙾 𝙰 𝙲𝙷𝙰𝚃 𝙶𝚁𝙾𝚄𝙿', url='http://t.me/donlee_robot?startgroup=true')
    ]]
...
async def admin(bot, msg):
    button = [[  
       Import.Button("𝙲𝙻𝙸𝙲𝙺➡️", url="t.me/TEAM_KERALA")
       ]]
    await msg.reply_text(
